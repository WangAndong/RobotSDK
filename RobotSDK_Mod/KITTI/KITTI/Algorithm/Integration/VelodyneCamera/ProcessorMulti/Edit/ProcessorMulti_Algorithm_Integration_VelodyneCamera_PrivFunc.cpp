...
bool DECOFUNC(processMultiInputData)(void * paramsPtr, void * varsPtr, QVector<QVector<void *> > inputParams, QVector<QVector<void *> > inputData, void * outputData, QList<int> & outputPortIndex)
{
	...
	*/
	...
	n=drawpoints.size();
	float step=vars->range/vars->colornum;
	for(i=0;i<n;i++)
	{
		int tmpi=int(scales[i]/vars->range);
		float r=(scales[i]-tmpi*vars->range)/vars->range;
		float g=0;
		if(g>1.0){g-=1.0;}
		float b=1-r;
		if(b>1.0){b-=1.0;};
		cv::circle(outputdata->image,drawpoints[i],vars->radius,cv::Scalar(255*r,255*g,255*b),-1);
	}
	return 1;
}
